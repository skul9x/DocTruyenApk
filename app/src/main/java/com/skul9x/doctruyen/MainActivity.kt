...
class MainActivity : AppCompatActivity() {
    ...
    private fun verifyHosting() {
        ...
        // If we have cookie, try load immediately without verify
        if (RetrofitClient.cookie.isNotEmpty()) {
            adapter.resetAnimations()
            loadStories(refresh = true)
            // Still verify in background just in case, or we can trust it works
            // Better strategy: Try load, if 403 then verify. 
            // For now let's just use it. If loadStories fails due to auth, we should handle it there.
            return
        }
    ...
    private fun loadStories(refresh: Boolean = false) {
        ...
        loadJob = lifecycleScope.launch {
            try {
                ...
                val isAuthError = e is com.google.gson.stream.MalformedJsonException || 
