...
object RetrofitClient {
    ...
    fun clearCookie(context: android.content.Context) {
        cookie = ""
        com.skul9x.doctruyen.utils.UserConfig.clearCookie(context)
    }
    
    /**
     * Detect if response body is an AES challenge from free.nf hosting
     * instead of actual JSON data
     */
    fun isChallengeResponse(body: String): Boolean {
        return body.contains("slowAES") || 
    ...
    private fun createClient(): OkHttpClient {
        ...
        return OkHttpClient.Builder()
            .addInterceptor { chain ->
                ...
                // REACTIVE CHECK: If response is the free.nf challenge (HTML with JS)
                // instead of our expected JSON, it means cookie died.
                if (isChallengeResponse(responseBody)) {
                     com.skul9x.doctruyen.utils.DebugLogger.logError("Network", "Detected free.nf Challenge/Cookie Expired!", null)
                     response.close() // Close the bad response
                     throw java.io.IOException("FreeNfChallenge")
                }
